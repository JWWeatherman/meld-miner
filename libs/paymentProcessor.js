    ...
    async.waterfall([
      ...
      function (workers, rounds, addressAccount, callback) {
        ...
        redisClient.multi(shareLookups).exec(function (error, allWorkerShares) {
          ...
          allWorkerShares = allWorkerShares.map(function (roundShare) {
            var resultForRound = {};

          Object.keys(roundShare).forEach(function (workerStr) {
            ...
            if(workerStr) {
              if (workerStr.indexOf(".") !== -1) {
                ...
                var workerInfo = workerStr.split('.');
                if (workerInfo.length === 2) {
                  //todo validate by daemon
                  var address = workerInfo[0];
