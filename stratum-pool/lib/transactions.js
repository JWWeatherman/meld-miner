...
var generateOutputTransactions = function (poolRecipient, recipients, rpcData, poolOptions) {
  ...
  if (rpcData.payee) {
    ...
    var masternodeReward = rpcData.masternode.amount;
    reward -= masternodeReward;
    rewardToPool -= masternodeReward;

    var masternodeScript = util.addressToScript(rpcData.masternode.payee);
    txOutputBuffers.push(Buffer.concat([
  ...
  if (rpcData.znode) {
    ...
    Object.keys(founders).forEach(function(founderAddress) {
